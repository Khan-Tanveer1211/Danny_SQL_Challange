DROP DATABASE IF EXISTS case_study_4_Data_Bank;
CREATE DATABASE case_study_4_Data_Bank;
USE case_study_4_Data_Bank;
...
VALUES
  ...
  SELECT * FROM CUSTOMER_NODES;  #3500 Rows
  SELECT * FROM CUSTOMER_TRANSACTIONS;  # 5868 Rows
  
 ####################CREATE MASTER TABLE ####################################
DROP TABLE IF EXISTS TK_DATA_BANK;
CREATE TABLE TK_DATA_BANK AS SELECT CT.CUSTOMER_ID,CT.txn_date,CT.txn_type,CT.txn_amount,CN.REGION_ID,R.REGION_NAME,
CN.NODE_ID,CN.START_DATE,CN.END_DATE FROM CUSTOMER_TRANSACTIONS CT 
LEFT OUTER JOIN CUSTOMER_NODES CN ON CT.CUSTOMER_ID = CN.CUSTOMER_ID
LEFT OUTER JOIN REGIONS R ON CN.REGION_ID=R.REGION_ID;
select count(*) from TK_DATA_BANK;


################## Question 1 : - How many unique nodes are there on the Data Bank system?

    SELECT COUNT(DISTINCT (NODE_ID)) FROM TK_DATA_BANK;
	
´################## Question 2 : -What is the number of nodes per region?

    SELECT CN.*,R.* FROM  CUSTOMER_NODES CN LEFT OUTER JOIN REGIONS R ON CN.REGION_ID=R.REGION_ID;
    SELECT R.REGION_NAME, COUNT(NODE_ID) FROM CUSTOMER_NODES CN LEFT OUTER JOIN REGIONS R ON CN.REGION_ID=R.REGION_ID GROUP BY REGION_NAME;


################## Question 3 : -How many customers are allocated to each region?

    SELECT REGION_NAME, COUNT( DISTINCT CUSTOMER_ID) FROM CUSTOMER_NODES CN INNER JOIN REGIONS R ON CN.REGION_ID=R.REGION_ID GROUP BY REGION_NAME;
	SELECT R.REGION_NAME, COUNT(DISTINCT CUSTOMER_ID) FROM CUSTOMER_NODES CN LEFT OUTER JOIN REGIONS R ON CN.REGION_ID=R.REGION_ID GROUP BY REGION_NAME;
	SELECT REGION_NAME, COUNT( DISTINCT CUSTOMER_ID) FROM TK_DATA_BANK GROUP BY REGION_NAME;

################## Question 4 : -How many days on average are customers reallocated to a different node?

    SELECT  AVG(datediff(end_date,start_date)) AS days FROM CUSTOMER_NODES WHERE END_DATE != '9999-12-31';

################## Question 5 : -What is the median, 80th and 95th percentile for this same reallocation days metric for each region?

    WITH relocation_cte AS (Select CN.*,R.Region_name, datediff(end_date, start_date) AS  diff_date from CUSTOMER_NODES CN left outer join regions R on CN.Region_id=R.Region_id WHERE end_date!='9999-12-31'),
	percentile_cte AS ( Select *,percent_rank() over(partition by region_name order by diff_date )*100 AS Perc  from relocation_cte)
	SELECT region_id,region_name, diff_date FROM percentile_cte where perc > 95 group by region_id, region_name,diff_date;

    WITH relocation_cte AS (Select CN.*,R.Region_name, datediff(end_date, start_date) AS  diff_date from CUSTOMER_NODES CN left outer join regions R on CN.Region_id=R.Region_id WHERE end_date!='9999-12-31'),
	percentile_cte AS ( Select *,percent_rank() over(partition by region_name order by diff_date )*100 AS Perc  from relocation_cte)
	SELECT region_id,region_name, diff_date FROM percentile_cte where perc > 80 group by region_id, region_name,diff_date;
	
	WITH relocation_cte AS (Select CN.*,R.Region_name, datediff(end_date, start_date) AS  diff_date from CUSTOMER_NODES CN left outer join regions R on CN.Region_id=R.Region_id WHERE end_date!='9999-12-31'),
	percentile_cte AS ( Select *,percent_rank() over(partition by region_name order by diff_date )*100 AS Perc  from relocation_cte)
	SELECT region_id,region_name, diff_date FROM percentile_cte where perc > 50 group by region_id, region_name,diff_date;
...
#SELECT count(distinct customer_id) FROM customer_transactions;
    ...
    SELECT SUM(X.CNT) FROM (SELECT  txn_type, sum(txn_amount), count(*) AS Cnt from customer_transactions group by txn_type) X;

################## Question 7 : -What is the average total historical deposit counts and amounts for all customers?

    select customer_id, AVG(txn_amount) from customer_transactions where txn_type = 'deposit' group by customer_id order by customer_id;
...
################## Question 8 : -For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
    ...
    SELECT * FROM (SELECT FIRST_MONTH, customer_id, COUNT(txn_type) as deposit_count from (select *,DATE_SUB(txn_date, Interval dayofmonth(txn_date)-1 DAY) AS FIRST_MONTH 
	from customer_transactions order by txn_date) X  WHERE txn_type= 'deposit' GROUP by FIRST_MONTH, customer_id) Y where deposit_count > 1 order by customer_id;

    WITH trans_cte AS (SELECT customer_id, month(txn_date) AS txn_month,
    SUM(IF(txn_type="deposit", 1, 0)) AS deposit_count,´
	SUM(IF(txn_type="withdrawal", 1, 0)) AS withdrawal_count,´
	SUM(IF(txn_type="purchase", 1, 0)) AS purchase_count FROM customer_transactions GROUP BY customer_id, month(txn_date) order by month(txn_date))
	SELECT txn_month, Count(DISTINCT CUSTOMER_ID) AS customer_count From trans_cte Where deposit_count > 1 AND ( withdrawal_count=1  or purchase_count=1) group by txn_month;

################## Question 9 : - What is the closing balance for each customer at the end of the month?

    WITH closing_cte As (SELECT Customer_id,SUM(CASE WHEN  txn_type= 'deposit' Then txn_amount Else -txn_amount END) AS Balance, month(txn_date) AS txn_month
	FROM customer_transactions group by customer_id, month(txn_date) order by customer_id)
	SELECT Customer_id,txn_month,balance , Sum(balance) OVER(PARTITION BY customer_id ORDER BY txn_month ROWS BETWEEN UNBOUNDED preceding AND CURRENT ROW) As closing_balance from closing_cte;
 
 
################## Question 10 : -What is the percentage of customers who increase their closing balance by more than 5%?	

    WITH closing_cte As (SELECT Customer_id,SUM(CASE WHEN  txn_type= 'deposit' Then txn_amount Else -txn_amount END) AS Balance, month(txn_date) AS txn_month
	FROM customer_transactions group by customer_id, month(txn_date) order by customer_id),
	Close_cte AS(SELECT Customer_id,txn_month,balance , Sum(balance) OVER(PARTITION BY customer_id ORDER BY txn_month ROWS BETWEEN UNBOUNDED preceding AND CURRENT ROW) As closing_balance from closing_cte),
	percent_cte AS(SELECT *,(100*(LAST_VALUE(closing_balance) OVER(PARTITION BY customer_id)-FIRST_VALUE(closing_balance) OVER (PARTITION BY Customer_ID))/(FIRST_VALUE(closing_balance) OVER (PARTITION BY Customer_ID))) AS diffvalue from close_cte)
	SELECT 100 *Count(distinct customer_id)/ (SELECT count(distinct customer_id) from customer_transactions)  AS percenatge_customer FROM Percent_cte where diffValue > 5;
